...
% Position of the sound source (randomly generated - will then be triangulated)
%{
x_random = rand()*0.8;
y_random = rand()*0.5;
source_position = [x_random,y_random];
%}
...
sigNoise = AddNoise(sigDelay, 0.4);
...
%app.CoordinateDisplayTextArea.Value = sprintf("Actual source position: X = %.4f, Y = %.4f\nLookup table estimation: X = %.4f, Y = %.4f", source_position(1), source_position(2), Estimated_position(1), Estimated_position(2));
%pause(1);
...
function noisySig = AddNoise(delayedSig, level)
noisySig = delayedSig + level*randn(size(delayedSig));
end
